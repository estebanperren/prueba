  ...
  <div className="card-body cartwidget">
    <div className="d-flex justify-content-between">
      <div className="d-flex flex-row align-items-center">
        <div>
          <img src={articulo.item.images[0]} className="img-fluid rounded-3" alt={articulo.item.title} style={{width: '80px'}} />
        </div>
    
        <div className="ms-3">
          <h5>{articulo.item.title}</h5>
          <p className="small mb-0 " style={{display: (props.description) ? "block" : "none"}}>{articulo.item.description}</p>
        </div>
      ...
      <div className="d-flex flex-row align-items-center">
        ...
        <div style={{width: '80px'}}>
          <h5 className="mb-0">${articulo.item.price * articulo.cantidad}</h5>
        </div>
